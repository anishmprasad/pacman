  ...
  if (this.dir == g.oppositeDirections[b]) {
    ...
    if (this.dir == 0) {
      ...
      if (c && c.allowedDir & b) {
        ...
        var f = 0;
        if (d[0] == this.tilePos[0] && d[1] == this.tilePos[1]) f = 1;else {
          d[c.axis] -= c.increment;
...
E.prototype.i = function (b) {
  ...
  if (h.intersection) switch (this.mode) {
    ...
    case 1:
    case 8:
      if ((this.dir & h.allowedDir) == 0 && h.allowedDir == g.oppositeDirections[this.dir]) this.nextDir = g.oppositeDirections[this.dir];else {
        b = 99999999999;
        c = 0;
        for (var j in i) {
          var k = i[j];
...
E.prototype.v = function () {
  ...
  if (this.routineMoveId == A[this.routineToFollow].length) if (this.mode == 16 && this.freeToLeavePen && !g.ghostExitingPenNow) {
    ...
    if (this.id == g.playerCount || this.freeToLeavePen) this.a(128);else {
...
google.pacman || function () {
    ...
    g.canvasClicked = function (b, c) {
        ...
        var f = g.getPlayfieldX(d.pos[1] + d.posDelta[1]) + 16,
            ...
            j = Math.abs(b - f),
            k = Math.abs(c - h);
        if (j > 8 && k < j) d.requestedDir = b > f ? 8 : 4;else if (k > 8 && j < k) d.requestedDir = c > h ? 2 : 1;
    };
    ...
    g.handleTouchMove = function (b) {
        if (b.touches.length > 1) g.cancelTouch();else {
    ...
    g.handleTouchEnd = function (b) {
        if (g.touchDX == 0 && g.touchDY == 0) g.canvasClicked(g.touchStartX, g.touchStartY);else {
            var c = Math.abs(g.touchDX),
                d = Math.abs(g.touchDY);
            if (c < 8 && d < 8) g.canvasClicked(g.touchStartX, g.touchStartY);else if (c > 15 && d < c * 2 / 3) g.actors[0].requestedDir = g.touchDX > 0 ? 8 : 4;else if (d > 15 && c < d * 2 / 3) g.actors[0].requestedDir = g.touchDY > 0 ? 2 : 1;
        }
        b.preventDefault();
        b.stopPropagation();
        g.cancelTouch();
    ...
    g.detectCollisions = function () {
        ...
        for (var b = g.playerCount; b < g.playerCount + 4; b++) {
            for (var c = 0; c < g.playerCount; c++) {
                if (g.actors[b].tilePos[0] == g.actors[c].tilePos[0] && g.actors[b].tilePos[1] == g.actors[c].tilePos[1]) if (g.actors[b].mode == 4) {
                    g.ghostDies(b, c);
                    return;
    ...
    g.updateCruiseElroySpeed = function () {
        ...
        if (!g.lostLifeOnThisLevel || g.actors[g.playerCount + 3].mode != 16) {
            var c = g.levels;
            if (g.dotsRemaining < c.elroyDotsLeftPart2) b = c.elroySpeedPart2 * 0.8;else if (g.dotsRemaining < c.elroyDotsLeftPart1) b = c.elroySpeedPart1 * 0.8;
        }
    ...
    g.changeGameplayMode = function (b) {
        g.gameplayMode = b;
        if (b != 13) for (var c = 0; c < g.playerCount + 4; c++) {
            g.actors[c].b();
        }switch (b) {
            case 0:
                g.playAmbientSound();
    ...
    g.cutsceneNextSequence = function () {
        ...
        if (g.cutscene.sequence.length == g.cutsceneSequenceId) g.stopCutscene();else {
    ...
    g.blinkEnergizers = function () {
        switch (g.gameplayMode) {
            ...
            case 14:
                ...
                break;
            default:
                if (g.globalTime % (g.timing[0] * 2) == 0) g.playfieldEl.className = "";else if (g.globalTime % (g.timing[0] * 2) == g.timing[0]) g.playfieldEl.className = "blk";
                break;
        }
    };
    g.blinkScoreLabels = function () {
        if (g.gameplayMode != 13) {
            var b = "";
            if (g.globalTime % (g.timing[17] * 2) == 0) b = "visible";else if (g.globalTime % (g.timing[17] * 2) == g.timing[17]) b = "hidden";
            if (b) for (var c = 0; c < g.playerCount; c++) {
    ...
    g.playDotEatingSound = function (b) {
        if (g.soundAvailable && google.pacManSound) if (g.gameplayMode == 0) if (g.dotEatingNow[b]) g.dotEatingNext[b] = a;else {
            if (b == 0) {
    ...
    g.playAmbientSound = function () {
        if (g.soundAvailable && google.pacManSound) {
            var b = 0;
            if (g.gameplayMode == 0 || g.gameplayMode == 1) b = g.ghostEyesCount ? "ambient-eyes" : g.mainGhostMode == 4 ? "ambient-fright" : g.dotsEaten > 241 ? "ambient-4" : g.dotsEaten > 207 ? "ambient-3" : g.dotsEaten > 138 ? "ambient-2" : "ambient-1";else if (g.gameplayMode == 13) b = "cutscene";
            if (b) try {
    ...
    g.g = function (b, c) {
        if (b < c) return -1;else if (b > c) return 1;
        return 0;
    };
